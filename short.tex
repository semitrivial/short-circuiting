...
circular-looking definition within this paper).
...
\[
\mbox{$``Enumerate all the EA-sentences which you know.''$}
\]
...
$X$ were asked, ``Do you know $\phi$?''
The reason why this would not work is because for certain mathematical statements,
the answer may depend on how long the AGI is allowed to think about the question.
For example, suppose $n$ is a natural number and we ask the AGI, ``Do you know that
the $n$th Turing machine eventually halts when run on empty input?'' For certain
particular values of $n$, the AGI might be able to answer this question in ad hoc
ways, but in general, by the unsolvability of the Halting Problem, the best the
AGI can do is to run the $n$th Turing machine for some time and see experimentally
whether it halts in that time. The AGI might
run a Turing machine for a million steps and suspect that it does not halt,
but cannot rule out the possibility that it would halt after \emph{two} million steps.
Thus, if we demand a simple yes-no answer to ``Do you know that the $n$th Turing machine
eventually halts when run on empty input?'', the answer will only partly be about
what the AGI knows, and partly will be about how patient the AGI is (or how
quickly the AGI performs calculations). By designing Definition \ref{maindef} the way
we have, we allow the AGI to take as long as it likes, dividing up its
calculations in parallel (a process known as \emph{dovetailing}) so that it can
effectively spend unbounded amounts of time pondering the truth value of one difficult
formula without putting every other formula on hold.
...
\begin{quote}
  ...
  is understood or misunderstood by a hearer (``an interpreter'').'
...
\begin{example}
  ...
  is true if and only if for every assignment from $s$ from variables to $\mathbb N$,
  $\exists x (\neg K(\phi)\wedge\neg K(\neg\phi))$ is true (with variables interpreted
  by $s$).
  \item
  By the semantics of $\exists$, the above is true if and only if there is some
  $n\in\mathbb N$ such that, for every assignment $s$ such that $s(x)=n$,
  $\neg K(\phi)\wedge\neg K(\neg\phi)$ is true (with variables interpreted by $s$).
  \item
  By Definition \ref{maindefextension}, this is the case if and only if there is
  some $n\in\mathbb N$ such that, for every assignment $s$ with $s(x)=n$,
  $X$ does not know $\phi^s$ (according to Definition \ref{maindef})
  and $X$ does not know $\neg\phi^s$ (according to Definition \ref{maindef}).
  \item
...
\begin{example}
  ...
  an intelligence measure for idealized mechanical knowing agents as follows.
  ...
  Using Definition \ref{maindef}
  to systematically perform said specialization, it becomes: ``The Intuitive Ordinal
  Intelligence of an AGI $X$ is the supremum of the set of ordinals $\alpha$
  such that there is some $c$ such that $X$ would include $\mathcal(\overline c)$
  in the resulting enumeration if we asked $X$ to enumerate all the statements
  that he knows and can express in $\mathscr L$, where $\mathscr L$ is a
  standard mathematical language containing a predicate symbol $O$ for the
  set of codes of computable ordinals.''
\end{example}
