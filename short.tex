...
We declare the AGI to know a certain EA-statement
if and only if, when commanded
(in plain English) to enumerate all the EA-statements that it knows, said AGI
...
ways, but in general, by the impossibility of the Halting Problem, the best the
AGI can do is to run the $n$th Turing machine for awhile and see experimentally
whether it halts. If the $n$th Turing machine does not halt, then the AGI might
run it for a million steps and suspect that it does not halt, but maybe if the AGI
had run it for two million steps, the Turing machine would have halted. Thus,
if we demand a simple yes-no answer to ``Do you know that the $n$th Turing machine
...
quickly it performs calculations). By designing Definition \ref{maindef} the way
we have, we allow the AGI to take as long as it likes, by dividing up its
calculations (a process known as \emph{dovetailing}) so that it can effectively
spend unbounded amounts of time pondering the truth value of a difficult formula
without putting all the other formulas on hold.
